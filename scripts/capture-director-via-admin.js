/**
 * Captura ecrãs específicos do Director usando a sessão Admin (que tem acesso total).
 */
...
const SECTIONS = [
  ...
  { name: '02-secretaria-hub',   path: '/secretaria-hub',          label: 'Painel da Secretaria' },
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    document.querySelectorAll('button').forEach(b => { if (b.textContent?.trim() === 'Agora não') b.click(); });
  }).catch(() => {});
...
(async () => {
  ...
  console.log('[auth] OK (admin token)');

  const results = [];
  for (const s of SECTIONS) {
    try {
      process.stdout.write(`[${s.name}] ${s.label} ... `);
      await page.goto(`${BASE}${s.path}`, { waitUntil: 'networkidle2', timeout: 20000 });
      await sleep(2000);
      await dismissOverlays(page);
      await sleep(300);
